"""
LSTM with Attention Mechanism for Weekly Terrorism Attack Forecasting

This model improves upon the baseline LSTM by adding an attention
mechanism that learns to focus on the most relevant time steps in the sequence.
This is particularly useful for terrorism forecasting where certain periods
(holidays, political events, prior attacks) are more predictive than others.
...
Expected Improvement: 10-15% RMSE reduction over baseline LSTM
...
from keras.layers import Input, LSTM, Dense, Dropout, Layer, Multiply, Permute, Reshape
...
class AttentionLayer(Layer):
    ...
    Computes attention weights for each time step and returns weighted sum.
    This allows the model to focus on the most relevant time steps.
    """
    ...
    def call(self, x):
        ...
        # Compute attention weights (softmax over time dimension)
        # a: (batch_size, time_steps, 1)
...
class LSTMAttentionModel:
    """
    LSTM model with Attention mechanism for terrorism forecasting
    """
    ...
    def build_model(self, input_shape):
        ...
        # First LSTM layer (smaller units to reduce memory)
        x = LSTM(32, activation='relu', return_sequences=True, name='lstm_1')(inputs)
        x = Dropout(0.2, name='dropout_1')(x)
        
        # Second LSTM layer
        x = LSTM(32, activation='relu', return_sequences=True, name='lstm_2')(x)
    ...
    def train(self, X_train, y_train, X_val, y_val, epochs=50, batch_size=32):
        ...
        Reduced batch_size from 64 to 32 for memory efficiency
        """
        print("\n" + "="*60)
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                filepath='src/models/checkpoints/lstm_attention_best.h5',
                monitor='val_loss',
                save_best_only=True,
                verbose=0
            )
    ...
    def plot_training_history(self, save_path='lstm_attention_training_history.png'):
        ...
        axes[0].set_title('LSTM Attention: Training History', fontsize=14, fontweight='bold')
    ...
    def plot_predictions(self, y_true, y_pred, save_path='lstm_attention_predictions.png'):
        ...
        axes[0].set_title('LSTM Attention: Predictions vs Actual', fontsize=14, fontweight='bold')
        axes[0].legend(fontsize=10)
        axes[0].grid(True, alpha=0.3)
...
def main():
    ...
    print("="*60)
    print("LSTM WITH ATTENTION - WEEKLY TERRORISM FORECASTING")
    print("="*60)
    ...
    # Prepare data - only exclude date/text columns, use data as-is from preprocessing
    ...
    # Train model
    epochs = config['model'].get('epochs', 50)  # Reduced from 100
    batch_size = config['model'].get('batch_size', 64)  # Increased from 32 for efficiency
    ...
    # Save model
    save_model(model.model, 'lstm_attention_weekly')
    
    # Save metrics
    metrics_df = pd.DataFrame([{
        'Model': 'LSTM_Attention_Weekly',
        'RMSE': metrics['RMSE'],
        'MAE': metrics['MAE'],
        'MSE': metrics['MSE'],
        'R2': metrics['R2'],
    ...
    save_table(metrics_df, 'lstm_attention_metrics', format='both')
    
    # Compare with baseline
    print("\n" + "="*60)
    print("COMPARISON WITH BASELINE")
    print("="*60)
    print(f"Baseline (Linear Regression) RMSE: 9.8858")
    print(f"LSTM Attention RMSE: {metrics['RMSE']:.4f}")
    
    improvement = (9.8858 - metrics['RMSE']) / 9.8858 * 100
    print(f"Improvement: {improvement:+.2f}%")
    
    if metrics['RMSE'] < 9.8858:
        print("\n🎉 LSTM Attention beats the baseline!")
    else:
        print("\n⚠️  LSTM Attention needs improvement")
    
    print("\n" + "="*60)
    print("✅ TRAINING COMPLETE")
    print("="*60)
    print("\nResults saved to:")
    print("  • Model: src/models/lstm_attention_weekly/")
    print("  • Figures: reports/figures/lstm_attention_*.png")
    print("  • Metrics: reports/tables/lstm_attention_metrics.csv")
